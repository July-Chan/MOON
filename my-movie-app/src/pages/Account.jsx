...
const Account = () => {
    ...
    const { t, i18n } = useTranslation(); // 🔥 Дістали i18n
    
    const userEmail = localStorage.getItem('userEmail');
    const userName = localStorage.getItem('userName') || t('defaultUserName', 'User');

    const [ratedMovies, setRatedMovies] = useState([]);

    // 🔥 ОНОВЛЕНИЙ БЛОК: Завантажуємо і ПЕРЕКЛАДАЄМО оцінки
    useEffect(() => {
        ...
        <div className="home-container" style={{ paddingTop: '80px' }}>
            ...
            {userEmail === 'musukabuka@gmail.com' && (
              <button 
                  ...
                  style={{
                      marginTop: '15px', padding: '8px 16px', backgroundColor: 'rgba(138, 63, 252, 0.2)',
                      color: '#b19cd9', border: '1px solid #8a3ffc', borderRadius: '20px', cursor: 'pointer',
                      fontSize: '13px', fontWeight: '600', transition: 'all 0.3s ease', display: 'flex',
                      alignItems: 'center', gap: '8px'
                  }}
                  onMouseOver={(e) => { e.target.style.backgroundColor = '#8a3ffc'; e.target.style.color = 'white'; }}
                  onMouseOut={(e) => { e.target.style.backgroundColor = 'rgba(138, 63, 252, 0.2)'; e.target.style.color = '#b19cd9'; }}
              >
